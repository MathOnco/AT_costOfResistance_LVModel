...
def GenerateATComparisonPlot(initialTumourSize, rFrac, paramDic,
                             ...
                             printDifferenceInTTP=False, markInitialSize=True,
    ...
    PlotSimulation(resultsDf, sizeVar="V", drugConcentrationVec=resultsDf.D, xlim=t_end,
                   ...
                   colorA=colorA, ax=ax)
    ax.vlines(x=ttp_ct, ymin=0, ymax=ylim, colors=[(1, 148 / 255, 9 / 255)], linestyles='-', linewidth=6)
    ax.vlines(x=ttp_AT, ymin=0, ymax=ylim, colors=[(9 / 255, 68 / 255, 134 / 255)], linestyles='--', linewidth=6)
    if markInitialSize: ax.hlines(xmin=0, xmax=t_end, y=initialTumourSize, linestyles=':', linewidth=6)
    ax.set_ylim(0, ylim)
    ax.set_xlabel("")
    ax.set_ylabel("")
    if not decorateX:
